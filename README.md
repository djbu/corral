...
**Status: pre-alpha — M8 complete (`v0.8.0`).** corral includes supervised interactive sessions, checkpoint/resume and idle reaping, headless task-DAG orchestration, opt-in remote access through TLS + bearer tokens, a verified per-repository permission-learning loop, safe daemon/service lifecycle, reproducible private releases, verified private/offline installation, hot backup, guarded restore, conservative GC and low-disk protection. M8 adds checked preflight plus explicit, auditable release or recoverable discard for isolated task worktrees. The `v0.7.0 → v0.8.0` upgrade and private Homebrew formula are exercised on clean macOS/Linux runners before promotion. Start with the human-facing [usage guide](docs/manual/GUIA_DE_USO.md), [architecture manual](docs/manual/ARQUITECTURA.md), or [private release runbook](docs/manual/RELEASES_PRIVADAS.md). The ordered work lives in the [post-M6 execution plan](docs/roadmap/POST_M6.md).
